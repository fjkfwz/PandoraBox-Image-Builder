...
#
# 	描述:MT7612v2/MT7610E/MT7612E uci无线脚本4e
#	功能：对接uci与mt7612配置文件，
#
#
...
#	reload_mt7612
	...
	for vif in $vifs; do
		...
		case "$mode" in
			adhoc)
				echo "mt7612 only support ap+wds+sta!"
			;;
			sta)
				[ ${mt7612_sta_num:-0} -ge 1 ] && {
					echo "mt7612 only support 1 sta!"
				}
				mt7612_sta_num=$(($mt7612_sta_num + 1))
				debug "mt7612_sta_num=$mt7612_sta_num"
			;;
			ap)
				mt7612_ap_num=$(($mt7612_ap_num + 1))
				debug "mt7612_ap_num=$mt7612_ap_num"
				apmode=1
				mt7612_ap_vif="${mt7612_ap_vif:+$mt7612_ap_vif }$vif"
			;;
			wds)
				config_get wds_addr "$vif" bssid
				[ -z "$wds_addr" ] || {
					wds_addr=$(echo "$wds_addr" | tr 'A-F' 'a-f')
					append mt7612_wds "$wds_addr"
				}
			;;
			monitor)
			;;
	...
	#开始准备该设备的无线配置参数
	mt7612_prepare_config $device
}


mt7612_prepare_config() {

	debug "mt7612_prepare_config"
...
#开始判断WiFi的MAC过滤方式.
	...
	*|disable|none|0)
	...
	#FIXME:单独的STA模式无法启动。
	[ "$mt7612_ap_num" = "0" ] && [ "$mt7612_sta_num" = "1" ] && {
	 ssid_num=1
	}
					
	cat > /tmp/MT7612.dat<<EOF
...
BssidNum=${ssid_num:-1}
SSID1=${ssid1:-PandoraBox_5G_SSID1}
SSID2=${ssid2:-PandoraBox_5G_SSID2}
SSID3=${ssid3:-PandoraBox_5G_SSID3}
SSID4=${ssid4:-PandoraBox_5G_SSID4}
SSID5=
SSID6=
SSID7=
SSID8=
...
enable_mt7612() {
	...
	ln -s /tmp/MT7612.dat /etc/Wireless/MT76X2/MT7612.dat 2>/dev/null
	}
...
#	mt7612_prepare_config $device
	...
	local mt7612_vifs mt7612_ap_vifs  mt7612_sta_vifs mt7612_wds_vifs
	
	for vif in $vifs; do
		debug "vifs vif=$vif"
		config_get mode "$vif" mode
		[ "$mode" == "ap" ] && {
			mt7612_ap_vifs="$mt7612_ap_vifs $vif"
		}
		[ "$mode" == "sta" ] && {
			mt7612_sta_vifs="$mt7612_sta_vifs $vif"
		}
		[ "$mode" == "wds" ] && {
			mt7612_wds_vifs="$mt7612_ap_vifs $vif"
		}
	done
	
	mt7612_vifs="$mt7612_vifs$mt7612_ap_vifs $mt7612_sta_vifs $mt7612_wds_vifs "
	
#	for vif in $vifs; do
	for vif in $mt7612_vifs; do
		config_get_bool disabled $vif disabled 0
		#如果关闭，则略过该vif配置
		[ "$disabled" == "1" ]&& {
			continue
...
#		config_get hidessid $vif hidden 0	
		...
		[ "$mode" == "sta" ] && {
					...
					[ "$mt7612_ap_num" = "0" ] && [ "$mt7612_sta_num" = "1" ] || [ "$if_num" == "-1" ]&& {
						#初始化phy
						ifconfig rai0 up
					}
...
#					ifconfig $ifname up
	...
	#配置WPS
	iwpriv $device set WscConfMode=0
	
	if [ "$wps" == "pbc" ]  && [ "$encryption" != "none" ]; then {
		enable_mt7612_wps_pbc $device
		touch /tmp/"$device"_wps_pbc.lock 2>/dev/null
	}
	else
		rm -rf /tmp/"$device"_wps_pbc.lock 2>/dev/null
	fi;
...
#detect_mt7612函数用于检测是否存在驱动
detect_mt7612() {
	local i=-1
#判断系统是否存在mt7612_ap，不存在则退出
	cd /sys/module/
	[ -d mt76x2e ] || return
...
config wifi-iface
	...
	option mode     ap
# 	option doth     1
	option ssid     PandoraBox_5G${i#0}_$( echo $(mt7612_get_mac Factory) | awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)
